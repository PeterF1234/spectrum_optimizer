...
def save_reference_spectrum(csvfile,lambdarange,ref_pad=50):
    X_ref, Y_ref = BO.read_expt(csvfile,ref_pad)
    ref_range = np.arange(min(X_ref),max(X_ref)+1)
    int_range = BO.get_intrange(lambdarange, ref_range)
    BO.save_expt(csvfile,ref_pad,int_range)
...
def opt_spect_molecule(folder_path): # folder path example: '/home/peter/Munka/bechmark/v1.0_multicore/PDI_2'
    ...
    save_reference_spectrum(ref_csv,lambdarange,ref_pad=50) # reformat the reference spectrum for the final database
    ...
    for i in error_functions:
        ...
        for logfile in files:
            if logfile[-4:] == '.log' or logfile[-4:] == '.out' or logfile[-4:] == '.dat':
                print(f'Working on file: {logfile}')
                print(f'Ref csv: {ref_csv}')
                print(f'Error directory: {error_dir}')
                BO.print_result(ref_csv,logfile,lambdarange,ref_pad,dumpfile=dumpfile_name,writeopt=True,errorfunc=i,workdir=error_dir,logscale_hmap=True)
            # sys.exit(1)
